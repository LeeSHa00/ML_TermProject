...
def get_various_encode_scale(X, numerical_columns, categorical_columns, scalers=None, 
                             encoders=None, scaler_name=None, encoder_name=None):
...
def MeanShifts(X, y, bandwidths=None, max_iters=None, n_job=None):
    ...
    if bandwidths is None:
        bandwidths = [estimate_bandwidth(X, quantile=0.25), estimate_bandwidth(X, quantile=0.75)]
    if max_iters is None:
        max_iters = [300]
    if n_job is None:
        n_job = -1
    ...
    for numerical_column, categorical_column in zip(numerical_columns, categorical_columns):
        ...
        for scaler in scalers:
            ...
            for encoder in encoders:
                ...
                silhouette_score, params, kmeans_centerDF_ex = kmeans(cleaned_x[i][j], y, max_cluster=max_cluster,
                ...
                silhouette_score, params, GMM_centerDF_ex = GMM(cleaned_x[i][j], y, max_cluster=max_cluster,
                ...
                silhouette_score, params, DBSCAN_centerDF_ex = DBSCANs(x[i][j], y, epsS=epsS, min_samples=min_samples,
                ...
                silhouette_score, params, MeanShift_centerDF_ex = MeanShifts(x[i][j], y, bandwidths=bandwidths,
